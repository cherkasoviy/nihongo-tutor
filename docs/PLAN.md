...
| Hosting | **Hetzner `CPX12`** (1 vCPU, 2 GB, 40 GB disk, Falkenstein, ~$14/mo incl. IPv4), Ubuntu 24.04 + Docker. *Changed from GCP `e2-medium`*: Hetzner's cost-optimised CX/CAX line has been out of stock everywhere since early September 2026 (DRAM shortage), and GCP's equivalent lands near $35/mo once the external IP and balanced PD are counted. Consequence of 2 GB: a **swapfile is required before the first image build** — `npm run build` under Vite is the memory peak, not the running stack (~1 GB idle). Nothing in the plan is host-specific, so a further move is a `pg_dump` + audio volume copy |
| AI | Claude via **Google Vertex AI** (content generation, roleplay, corrections); Google Cloud TTS; **Google Cloud Speech-to-Text v2** for STT. *Changed from the direct Anthropic API and OpenAI transcription*: direct Anthropic billing is not available to the owner, and neither is OpenAI's. All three authenticate with one Google service account (`roles/aiplatform.user` plus the TTS/STT roles), so **no Anthropic or OpenAI key exists anywhere in the system**. Off GCP there is no attached service account to inherit, so it is a JSON key on the box — keep it out of the repo. STT v2 gives 60 min/month free, takes `OGG_OPUS` natively, and its PhraseSet adaptation replaces the `prompt=<target sentence>` trick |
...
2. One Google Cloud project with billing enabled, the Vertex AI, Text-to-Speech and Speech-to-Text APIs turned on, and a service account holding `roles/aiplatform.user` plus the TTS/STT roles, downloaded as a JSON key for the box. That single credential covers all three; no Anthropic or OpenAI key is needed.
3. A Hetzner CPX12 (or any Docker host) with a public IP and a domain pointing at it (Caddy needs a public hostname for TLS + webhook), plus a **2 GB swapfile before the first image build**. Secrets go only in `.env` on the server, never in the repo.
