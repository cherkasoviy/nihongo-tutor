...
3. A Hetzner CPX12 (or any Docker host) with a public IP and a domain pointing at it (Caddy needs a public hostname for TLS + webhook), plus a **2 GB swapfile before the first image build**. A GCP project with billing enabled for the Text-to-Speech and Speech-to-Text APIs, and a service-account JSON key on the box — off-GCP there is no attached service account to inherit. Secrets go only in `.env` on the server, never in the repo.
